...
/**
 * 功能说明：测试读写锁: 写锁, 悲观读锁, 乐观读
 * 比较读写锁与乐观读锁的性能差异
 * 开发人员：@Author MaLi
 * 开发日期：@Date  2021/5/10
 */
public class Cache_WithStampedLock<K, V> {
    ...
    public V getValue(K key) {
        long stamp = stamptedLock.readLock();
        V value;
        try {
            value = map.get(key);
        } finally {
    ...
    public void testCache() {
        Cache_WithStampedLock<Integer, Integer> cache = new Cache_WithStampedLock<Integer, Integer>();
    ...
    private void caseMethod1(Cache_WithStampedLock<Integer, Integer> cache) {
    ...
    public void testCase() {
        TestCase.run(Cache_WithStampedLock.class);
    }
}
